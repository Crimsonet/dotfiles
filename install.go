...
func main() {
	...
	err = copyFile("hypr/hyprlock.conf.bak", filepath.Join(os.Getenv("HOME"), ".config/hypr/hyprlock.conf.bak"))
	if err == nil {
		fmt.Printf("[%s] >>> hypr/hyprlock.conf.bak has been installed.\n", time.Now().Format("15:04:05"))
	} else {
		fmt.Println("Error copying hypr/hyprlock.conf.bak:", err)
	}
